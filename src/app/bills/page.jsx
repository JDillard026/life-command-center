...
function addMonthsClamped(iso, monthsToAdd) {
  ...
  const next = new Date(
    dt.getFullYear(),
    dt.getMonth() + Number(monthsToAdd || 0),
    1,
    12,
    0,
    0,
...
function isBillPaidThisCycle(bill) {
  ...
  const cycleStart = cycleStartForBill(bill);
  const cycleEnd = cycleEndForBill(bill);
  const startSerial = isoSerial(cycleStart);
  const endSerial = isoSerial(cycleEnd);
...
function billStatusMeta(bill) {
  ...
  if (bill.active === false) {
    return { label: "Inactive", tone: "neutral", percent: 0, isPaid: false };
  }

  if (isBillPaidThisCycle(bill)) {
    return { label: "Paid", tone: "green", percent: 100, isPaid: true };
  }
...
function emptyDebtDraft(defaultAccountId = "") {
  return {
    isDebtBill: false,
    debtMode: "none",
    linkedDebtId: "",
...
function buildEmptyBillForm(defaultAccountId = "") {
  return {
    ...
    autopay: false,
    lastPaidDate: "",
    ...emptyDebtDraft(defaultAccountId),
  };
}

function hydrateDebtCreationDefaults(draft, fallbackAccountId = "") {
  const parsedAmount = parseMoneyInput(draft.amount);
  return {
    ...draft,
    newDebtName: draft.newDebtName || draft.name || "",
    newDebtMinPay:
      draft.newDebtMinPay ||
      (Number.isFinite(parsedAmount) && parsedAmount > 0
        ? String(round2(parsedAmount))
        : ""),
    newDebtFrequency: draft.newDebtFrequency || draft.frequency || "monthly",
    newDebtDueDate: draft.newDebtDueDate || draft.dueDate || isoDate(),
    newDebtCategory: draft.newDebtCategory || draft.category || "",
    newDebtNotes: draft.newDebtNotes || draft.notes || "",
    newDebtAccountId:
      draft.newDebtAccountId || draft.accountId || fallbackAccountId || "",
    newDebtAutopay: draft.newDebtAutopay === true || draft.autopay === true,
  };
}
...
function buildBillEditorState(bill, linkedDebt, defaultAccountId = "") {
  ...
  const parsedBillAmount = safeNum(bill.amount, 0);
  ...
  return {
    ...
    newDebtBalance:
      linkedDebt && Number.isFinite(linkedDebt.balance)
        ? String(linkedDebt.balance)
        : "",
    newDebtAprPct:
      linkedDebt && Number.isFinite(linkedDebt.aprPct)
        ? String(linkedDebt.aprPct)
        : "",
    newDebtMinPay:
      linkedDebt && Number.isFinite(linkedDebt.minPay)
        ? String(linkedDebt.minPay)
        : parsedBillAmount > 0
        ? String(parsedBillAmount)
        : "",
    newDebtExtraPay:
      linkedDebt && Number.isFinite(linkedDebt.extraPay)
        ? String(linkedDebt.extraPay)
        : "",
    ...
    newDebtAccountId:
      linkedDebt?.accountId || bill.accountId || defaultAccountId || "",
    newDebtAutopay: linkedDebt
      ? linkedDebt.autopay === true
      : bill.autopay === true,
  };
}
...
function IconGhostBtn({ children, onClick, title, danger = false, disabled = false }) {
  return (
    <button
      type="button"
      className={`billIconGhost${danger ? " billIconGhostDanger" : ""}`}
      onClick={onClick}
      title={title}
      aria-label={title}
      disabled={disabled}
...
function DebtSetupSection({
  ...
  const usableAccounts = accounts.filter(
    (account) => !isInvestmentAccountType(account.type)
  );

  const selectedDebt =
    debtProfiles.find((debt) => debt.id === value.linkedDebtId) || null;

  function turnIntoDebtBill() {
    setValue((prev) => {
      const nextMode =
        prev.debtMode && prev.debtMode !== "none"
          ? prev.debtMode
          : debtProfiles.length
          ? "link_existing"
          : "create_new";
      ...
      const hydrated =
        nextMode === "create_new"
          ? hydrateDebtCreationDefaults(prev, defaultAccountId)
          : prev;
  ...
  function switchToLinkExisting() {
    setValue((prev) => ({
      ...prev,
      isDebtBill: true,
      debtMode: "link_existing",
    }));
  }
  ...
  return (
    <SectionCard
      title="Debt Setup"
      subcopy="Keep fixed bills simple or connect the bill to a real debt profile."
    >
      <div className="billInlineTools">
        <ActionBtn
          variant={!value.isDebtBill ? "primary" : "ghost"}
          onClick={turnIntoFixedBill}
        >
          Fixed bill
        </ActionBtn>
        ...
        <ActionBtn
          variant={value.isDebtBill ? "primary" : "ghost"}
          onClick={turnIntoDebtBill}
        >
          Debt / payoff bill
        </ActionBtn>
      </div>

      <div className="billInlineHint">
        Debt bills can link to an existing debt profile or create a new one that
        appears on the Debt page.
      </div>
        ...
        <div className="billFormStack" style={{ marginTop: 14 }}>
          ...
          {value.debtMode === "link_existing" ? (
            <div>
              ...
              <select
                className="billField billFieldStrong"
                value={value.linkedDebtId}
                onChange={(e) =>
                  setValue((prev) => ({
                    ...prev,
                    linkedDebtId: e.target.value,
                  }))
                }
              >
                ...
                {debtProfiles.map((debt) => (
                  <option key={debt.id} value={debt.id}>
                    {debt.name} • {money(debt.balance)} • {safeNum(debt.aprPct, 0)}%
                    APR
                  </option>
                ))}
              </select>
              ...
              <div className="billInlineHint">
                {selectedDebt
                  ? `Linked to ${selectedDebt.name}. Balance ${money(
                      selectedDebt.balance
                    )} • minimum ${moneyTight(selectedDebt.minPay)} • extra ${moneyTight(
                      selectedDebt.extraPay
                    )}`
                  : "Pick the debt this bill should pay down."}
              </div>
            </div>
          ) : null}
          ...
          {value.debtMode === "create_new" ? (
            <>
              <div className="billFormGrid4">
                <div>
                  <div className="billTinyLabel">Debt name</div>
                  <input
                    className="billField billFieldStrong"
                ...
                <div>
                  <div className="billTinyLabel">Current balance</div>
                  <input
                    className="billField billFieldStrong"
                    inputMode="decimal"
                ...
                <div>
                  <div className="billTinyLabel">APR %</div>
                  <input
                    className="billField billFieldStrong"
                    inputMode="decimal"
                ...
                <div>
                  <div className="billTinyLabel">Minimum payment</div>
                  <input
                    className="billField billFieldStrong"
                    inputMode="decimal"
              ...
              <div className="billFormGrid4">
                <div>
                  <div className="billTinyLabel">Extra payment</div>
                  <input
                    className="billField billFieldStrong"
                    inputMode="decimal"
                    ...
                    onChange={(e) =>
                      setValue((prev) => ({
                        ...prev,
                        newDebtExtraPay: e.target.value,
                      }))
                    }
                    placeholder="0.00"
                  />
                ...
                <div>
                  <div className="billTinyLabel">Debt due date</div>
                  <input
                    type="date"
                    className="billField billFieldStrong"
                ...
                <div>
                  <div className="billTinyLabel">Debt frequency</div>
                  <select
                    className="billField billFieldStrong"
                    ...
                    onChange={(e) =>
                      setValue((prev) => ({
                        ...prev,
                        newDebtFrequency: e.target.value,
                      }))
                    }
                  >
                ...
                <div>
                  <div className="billTinyLabel">Debt account</div>
                  <select
                    className="billField billFieldStrong"
                    ...
                    onChange={(e) =>
                      setValue((prev) => ({
                        ...prev,
                        newDebtAccountId: e.target.value,
                      }))
                    }
                  >
              ...
              <div className="billFormGrid2">
                <div>
                  <div className="billTinyLabel">Debt category</div>
                  <input
                    className="billField billFieldStrong"
                    value={value.newDebtCategory}
                    onChange={(e) =>
                      setValue((prev) => ({
                        ...prev,
                        newDebtCategory: e.target.value,
                      }))
                    }
                ...
                <div>
                  <div className="billTinyLabel">Debt autopay</div>
                  <div className="billInlineTools">
                    <ActionBtn
                      variant={value.newDebtAutopay ? "primary" : "ghost"}
                      onClick={() =>
                        setValue((prev) => ({
                          ...prev,
                          newDebtAutopay: true,
              ...
              <div>
                <div className="billTinyLabel">Debt notes</div>
                <textarea
                  className="billField billFieldStrong"
                  rows={4}
                  ...
                  onChange={(e) =>
                    setValue((prev) => ({
                      ...prev,
                      newDebtNotes: e.target.value,
                    }))
                  }
                  placeholder="Optional debt note..."
                />
              </div>

              <div className="billInlineHint">
                Saving this bill will also create a new debt profile on the Debt
                page and auto-link the two together.
              </div>
            </>
          ) : null}
        </div>
      ...
      <div className="billEmptyState billInlineEmpty">
        <div>
          ...
          <div className="billEmptyText">
            Use the payment box to log a payment on this bill.
          </div>
        </div>
      </div>
    );
    ...
    <div className="billModalRoot">
      ...
      <div className="billModalPanel">
        <div className="billModalHeader">
          ...
          <IconGhostBtn onClick={onClose} title="Close history">
            <X size={16} />
          </IconGhostBtn>
        </div>
...
function DueTile({ bill, status }) {
  ...
  const topLine = status?.isPaid
    ? bill?.lastPaidDate
      ? `Paid ${shortDate(bill.lastPaidDate)}`
      : "Paid"
  ...
  return (
    ...
    >
      <div
        className="billDueTileIcon"
        style={{
          color: status?.tone === "neutral" ? "#fff" : meta.text,
          background: meta.bg,
          borderColor: meta.border,
        }}
      >
      ...
      <div
        className="billDueTileMeta"
        style={{ color: status?.tone === "neutral" ? "rgba(255,255,255,0.7)" : meta.text }}
      >
        {bottomLine}
      </div>
    </div>
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div className="billCompactSub">
          {bill.category || "No category"} • {bill.frequency} • Monthly pressure {money(monthly)} • Updated {fmtAgo(bill.updatedAt)}
        </div>
      </div>
...
function BillSummaryCard({
  ...
  if (!bill) {
    return (
      <GlassPane size="card">
        ...
        />
        <div className="billEmptyState" style={{ minHeight: 260 }}>
          <div>
  ...
  const meta = toneMeta(status.tone);
  const linkedDebt =
    debtProfiles.find((debt) => debt.id === bill.linkedDebtId) || null;
  const accountNameById = new Map(accounts.map((a) => [a.id, a.name]));
  const monthlyPressure = monthlyWeight(bill.amount, bill.frequency);
  const debtPayoff = linkedDebt
    ? payoffLabel(
        linkedDebt.balance,
        linkedDebt.aprPct,
        safeNum(linkedDebt.minPay, 0) + safeNum(linkedDebt.extraPay, 0)
      )
    : "—";
  const debtInterest = linkedDebt
    ? estimatedMonthlyInterest(linkedDebt.balance, linkedDebt.aprPct)
    : 0;
  ...
  return (
    <GlassPane tone={status.tone} size="card" style={{ height: "100%" }}>
      <PaneHeader
        ...
        subcopy="Summary first. Edit only when you actually need it."
        right={
          <div className="billFocusHeaderTools">
      ...
      <div className="billFocusBox">
        ...
        <div style={{ marginTop: 14 }}>
          <ProgressBar fill={status.percent} tone={status.tone} />
        </div>

        <div className="billInfoGrid" style={{ marginTop: 14 }}>
          <div className="billInfoCell">
            <div className="billTinyLabel">Status</div>
            <div className="billInfoValue">{status.label}</div>
            <div className="billInfoSub">
              {status.isPaid
                ? bill.lastPaidDate
                  ? `Paid ${shortDate(bill.lastPaidDate)}`
                  : "Paid this cycle"
                : compactDueText(daysUntil(bill.dueDate))}
            </div>
          </div>

          <div className="billInfoCell">
            <div className="billTinyLabel">Due Cycle</div>
            <div className="billInfoValue">{shortDate(bill.dueDate)}</div>
            <div className="billInfoSub">
              {FREQUENCY_OPTIONS.find((opt) => opt.value === bill.frequency)?.label || bill.frequency}
            </div>
          </div>
          ...
          <div className="billInfoCell">
            ...
            <div className="billInfoValue">
              {accountNameById.get(bill.accountId) || "None"}
            </div>
            <div className="billInfoSub">Default pay-from account</div>
          </div>

          <div className="billInfoCell">
            <div className="billTinyLabel">Linked Debt</div>
            <div className="billInfoValue">{linkedDebt?.name || "None"}</div>
            <div className="billInfoSub">
              {linkedDebt
                ? `Balance ${money(linkedDebt.balance)}`
                : "No debt profile attached"}
            </div>
          </div>

          <div className="billInfoCell">
            <div className="billTinyLabel">Last Paid</div>
            <div className="billInfoValue">{shortDate(bill.lastPaidDate)}</div>
            <div className="billInfoSub">
              {payments.length
                ? `${payments.length} logged payment${payments.length === 1 ? "" : "s"}`
                : "No payment history"}
            </div>
          </div>

          <div className="billInfoCell">
            <div className="billTinyLabel">Monthly Pressure</div>
            <div className="billInfoValue">{money(monthlyPressure)}</div>
            <div className="billInfoSub">Estimated current-cycle bill load</div>
          </div>
        </div>
        ...
        <div className="billSummarySplit">
          <SectionCard
            title="Make Payment"
            subcopy="A cleaner payment box with the history moved out into a popup."
            right={
              <ActionBtn onClick={onOpenHistory}>
                <Receipt size={14} />
                History
              </ActionBtn>
            }
          >
            <div className="billPaymentSectionHead">
              ...
              <div className="billInlineHint">
                Log the payment here. The bill row will flip to paid when the payment lands in the active cycle.
              </div>
            </div>
              ...
              <div>
                ...
                >
                  ...
                  {paymentAccounts.map((account) => (
                    <option key={account.id} value={account.id}>
                      {account.name} • {money(account.balance)} • {accountTypeLabel(account.type)}
                    </option>
                  ))}
                </select>
              </div>

              <div className="billPaymentNoteSpan">
            ...
            <div className="billPaymentActionBar">
              ...
              >
                ...
                {paymentBusy ? "Saving..." : "Make Payment"}
              </ActionBtn>
            </div>
          </SectionCard>

          <SectionCard
            title="Linked Debt Snapshot"
            subcopy="If this bill is tied to debt, keep the debt pulse visible here."
          >
            {linkedDebt ? (
              <div className="billDebtSnapshot">
                <div className="billDebtSnapshotRow">
                  <span>Debt name</span>
                  <strong>{linkedDebt.name}</strong>
                </div>
                <div className="billDebtSnapshotRow">
                  <span>Balance</span>
                  <strong>{money(linkedDebt.balance)}</strong>
                </div>
                <div className="billDebtSnapshotRow">
                  <span>APR</span>
                  <strong>{safeNum(linkedDebt.aprPct, 0)}%</strong>
                </div>
                <div className="billDebtSnapshotRow">
                  <span>Interest drag</span>
                  <strong>{moneyTight(debtInterest)}/mo</strong>
                </div>
                <div className="billDebtSnapshotRow">
                  <span>Payoff</span>
                  <strong>{debtPayoff}</strong>
                </div>
              </div>
            ) : (
              <div className="billEmptyMini">
                This bill is not connected to a debt profile.
              </div>
            )}
          </SectionCard>
        </div>
      ...
      <ModalShell
        open={historyOpen}
        title={`${bill.name || "Bill"} Payment History`}
        subcopy="Clean history popup so the center card can stay focused on action."
        onClose={onCloseHistory}
        footer={<ActionBtn onClick={onCloseHistory}>Close</ActionBtn>}
      >
        <PaymentHistory
          payments={payments}
...
function BillDrawer({
  ...
  return (
    <DrawerShell
      ...
      subcopy={
        mode === "add"
          ? "Create a clean new bill without stretching the page."
          : "Edit the bill here, save, and get back to the summary view."
      }
      onClose={onClose}
      footer={
        <>
    ...
    >
      <div className="billFormStack">
        <SectionCard
          title="Bill Core"
          subcopy="Primary bill info, schedule, and default account."
        >
          <div className="billFormStack">
            <div>
            ...
            <div>
              <div className="billTinyLabel">Autopay</div>
              <div className="billInlineTools">
                <ActionBtn
                  variant={form.autopay ? "primary" : "ghost"}
                  onClick={() =>
                    setForm((prev) => ({ ...prev, autopay: true }))
                  }
                >
                  On
                </ActionBtn>
                ...
                <ActionBtn
                  variant={!form.autopay ? "primary" : "ghost"}
                  onClick={() =>
                    setForm((prev) => ({ ...prev, autopay: false }))
                  }
                >
                  Off
                </ActionBtn>
...
export default function BillsPage() {
  ...
  const paymentAccounts = useMemo(
    () => accounts.filter((account) => !isInvestmentAccountType(account.type)),
    [accounts]
  );

  const refreshPage = useCallback(
    async (preferredBillId = "") => {
      if (!supabase) {
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const [billsRes, debtsRes, accountsRes, settingsRes, paymentsRes] =
          await Promise.all([
            supabase
              .from("bills")
              .select("*")
        ...
        setSelectedBillId((prev) => {
          if (preferredBillId && loadedBills.some((bill) => bill.id === preferredBillId)) {
            return preferredBillId;
          }
          if (prev && loadedBills.some((bill) => bill.id === prev)) {
            return prev;
          }
        ...
        setAddForm((prev) => ({
          ...
          newDebtAccountId: prev.newDebtAccountId || primaryAccountId,
        }));
      } catch (err) {
  ...
  const selectedLinkedDebt = useMemo(() => {
    if (!selectedBill?.linkedDebtId) return null;
    return debtProfiles.find((debt) => debt.id === selectedBill.linkedDebtId) || null;
  ...
  const metrics = useMemo(() => {
    ...
    const nextBill = [...activeBills].sort((a, b) => {
    ...
    return {
      ...
      linkedDebtCount: linkedDebtBills.length,
      monthlyPressure,
  ...
  async function applyAccountDelta({
    ...
    if (!accountId) {
      return { ok: true, previousBalance: null, balance: null };
    }
    ...
    const previousBalance = Number.isFinite(startingBalance)
      ? safeNum(startingBalance, 0)
    ...
    if (accountError) {
      return {
        ok: false,
        message: accountError.message || "Could not update account balance.",
      };
    }
    ...
    const { error: ledgerError } = await supabase.from("account_transactions").insert({
      ...
      kind,
      amount: round2(Math.abs(safeNum(delta, 0))),
      delta: round2(delta),
      ...
      source_id: sourceId,
      created_at: ledgerTimestampFromIso(effectiveDate),
    });
    ...
    if (ledgerError) {
      ...
      return {
        ok: false,
        message: ledgerError.message || "Could not write account ledger.",
      };
    }
  ...
  function getPaymentAccountEffect(accountId, amount) {
    const account = accounts.find((row) => row.id === accountId);
    if (!account) {
      return { delta: 0, kind: "bill_payment" };
    }
  ...
  async function createDebtProfileFromDraft(draft) {
    ...
    if (!Number.isFinite(minPay) || minPay < 0) {
      throw new Error("Debt minimum payment must be 0 or greater.");
    }

    const extraPay =
      Number.isFinite(extraPayRaw) && extraPayRaw >= 0 ? extraPayRaw : 0;
    ...
    const res = await supabase
      ...
      .insert({
        ...
        amount: round2(
          Number.isFinite(billAmount) && billAmount > 0 ? billAmount : minPay
        ),
        active: true,
        ...
        account_id:
          draft.newDebtAccountId || draft.accountId || defaultAccountId || null,
        linked_debt_id: null,
        last_paid_date: null,
    ...
    if (res.error) {
      throw new Error(res.error.message || "Could not create debt profile.");
    }
  ...
  async function resolveLinkedDebtIdFromDraft(draft) {
    ...
    if (draft.debtMode === "link_existing") {
      if (!draft.linkedDebtId) {
        throw new Error("Choose an existing debt profile to link.");
      }
  ...
  async function addBill() {
    ...
    if (!name) {
      setPageError("Bill name is required.");
      return;
    }
    ...
    try {
      ...
      createdDebtId =
        addForm.isDebtBill && addForm.debtMode === "create_new" ? linkedDebtId : "";
      ...
      const res = await supabase
        ...
        .insert({
          ...
          due_date: nextBill.dueDate || null,
          amount: round2(nextBill.amount),
          active: nextBill.active !== false,
          balance: round2(nextBill.balance),
          min_pay: 0,
          extra_pay: 0,
          apr_pct: 0,
          autopay: nextBill.autopay === true,
          category: nextBill.category || "",
          notes: nextBill.notes || "",
          account_id: nextBill.accountId || null,
          linked_debt_id: nextBill.linkedDebtId || null,
          last_paid_date: null,
          created_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
        })
      ...
      if (res.error) {
        throw new Error(res.error.message || "Could not add bill.");
      }
      ...
      setDrawerMode(null);
      setStatus(
        saved.linkedDebtId ? "Bill added and debt linked." : "Bill added."
      );
  ...
  async function saveSelectedBill() {
    ...
    try {
      ...
      createdDebtId =
        editor.isDebtBill && editor.debtMode === "create_new" ? linkedDebtId : "";
      ...
      if (res.error) {
        throw new Error(res.error.message || "Could not save bill.");
      }
  ...
  async function deleteBillById(billId, billName = "this bill") {
    ...
    if (typeof window !== "undefined" && !window.confirm(`Delete ${billName}?`)) {
      return false;
    }
    ...
    if (relatedPayments.length) {
      ...
      if (paymentsError) {
        setPageError(paymentsError.message || "Could not delete related payments.");
        return false;
      }
    }
  ...
  async function makeBillPayment() {
    ...
    const shouldAdvanceDue =
      selectedBill.frequency !== "one_time" &&
      shouldAdvanceDueDateForPayment(selectedBill, paymentDate, paymentDraft.advanceDue);

    const nextDueDate = shouldAdvanceDue
      ? nextDueDateFromFrequency(
          paymentDate || selectedBill.dueDate || isoDate(),
          selectedBill.frequency
        )
      : selectedBill.dueDate || "";

    const nextLastPaidDate =
    ...
    const linkedDebt =
      debtProfiles.find((row) => row.id === selectedBill.linkedDebtId) || null;

    const nextLinkedDebtBalance = linkedDebt
      ? Math.max(0, round2(safeNum(linkedDebt.balance, 0) - amount))
      : 0;

    const nextLinkedDebtLastPaidDate =
      linkedDebt &&
      (!linkedDebt.lastPaidDate ||
        compareIsoDates(paymentDate, linkedDebt.lastPaidDate) > 0)
        ? paymentDate
        : linkedDebt?.lastPaidDate || "";

    const shouldAdvanceLinkedDebtDue =
      linkedDebt &&
      linkedDebt.frequency !== "one_time" &&
      shouldAdvanceDueDateForPayment(linkedDebt, paymentDate, paymentDraft.advanceDue);

    const nextLinkedDebtDueDate =
      linkedDebt && shouldAdvanceLinkedDebtDue
        ? nextDueDateFromFrequency(
            paymentDate || linkedDebt.dueDate || isoDate(),
            linkedDebt.frequency
          )
        : linkedDebt?.dueDate || "";
    ...
    try {
      ...
      if (payAccountId) {
        ...
        accountResult = await applyAccountDelta({
          accountId: payAccountId,
          delta: accountEffect.delta,
          kind: accountEffect.kind,
          note: `${selectedBill.name || "Bill"}${payNote ? ` • ${payNote}` : ""}`,
        ...
        if (!accountResult.ok) {
          throw new Error(accountResult.message || "Could not update payment account.");
        }
      }
      ...
      const { error: billError } = await supabase
        .from("bills")
        .update({
          last_paid_date: nextLastPaidDate || null,
      ...
      if (linkedDebt) {
        const { error: linkedDebtError } = await supabase
          .from("bills")
          .update({
            balance: nextLinkedDebtBalance,
            last_paid_date: nextLinkedDebtLastPaidDate || null,
            due_date: nextLinkedDebtDueDate || null,
            updated_at: new Date().toISOString(),
          })
          .eq("id", linkedDebt.id)
          .eq("user_id", userId);
      ...
      if (accountEffect && accountResult?.ok) {
        await applyAccountDelta({
          ...
          note: `${selectedBill.name || "Bill"} payment rollback`,
  ...
  async function deletePayment(payment) {
    ...
    const remainingLinkedDebtPayments = linkedDebt
      ? payments
          .filter(
            (row) =>
              row.id !== payment.id &&
              (row.linkedDebtId === linkedDebt.id || row.billId === linkedDebt.id)
          )
          .sort(
            (a, b) =>
              compareIsoDates(b.paymentDate, a.paymentDate) ||
    ...
    try {
      if (payment.accountId) {
        ...
        reversalResult = await applyAccountDelta({
          accountId: payment.accountId,
          delta: -originalEffect.delta,
          kind: "bill_payment_delete",
          note: `${currentBill?.name || "Bill"} payment deleted${payment.note ? ` • ${payment.note}` : ""}`,
        ...
        if (!reversalResult.ok) {
          throw new Error(reversalResult.message || "Could not reverse payment account.");
        }
      }
      ...
      if (currentBill) {
        ...
        const billPatch = {
          ...
          updated_at: new Date().toISOString(),
          balance:
            safeNum(currentBill.balance, 0) > 0
              ? round2(safeNum(currentBill.balance, 0) + safeNum(payment.amount, 0))
        ...
        if (
          ...
          billPatch.due_date = previousDueDateFromFrequency(
            currentBill.dueDate,
      ...
      if (linkedDebt) {
        ...
        const linkedDebtPatch = {
          last_paid_date: nextDebtLastPaidDate || null,
          balance: round2(safeNum(linkedDebt.balance, 0) + safeNum(payment.amount, 0)),
          updated_at: new Date().toISOString(),
        };
        ...
        if (
          ...
          linkedDebtPatch.due_date = previousDueDateFromFrequency(
            linkedDebt.dueDate,
            linkedDebt.frequency
          );
        }

        const { error: updateLinkedDebtError } = await supabase
          .from("bills")
          .update(linkedDebtPatch)
          .eq("id", linkedDebt.id)
          .eq("user_id", userId);
      ...
      if (deletePaymentError) {
        ...
        if (payment.accountId && originalEffect && reversalResult?.ok) {
          await applyAccountDelta({
            ...
            sourceId: payment.id,
            effectiveDate: isoDate(),
            startingBalance: reversalResult.balance,
          });
        }
      ...
      setStatus(linkedDebt ? "Payment deleted and debt resynced." : "Payment deleted.");
  ...
  const nextDueTone = metrics.nextBill
    ? dueMeta(daysUntil(metrics.nextBill.dueDate)).tone
    : "neutral";
        ...
        <div className="billPageShell">
            ...
            <div className="billHeroGrid">
              <div style={{ minWidth: 0 }}>
                ...
                <div className="billHeroSub">
                  Cleaner summary-first bills workspace with payment tools up front,
                  while full editing and adding now live in polished slide-over panels.
                </div>
              ...
              <div className="billHeroSide">
                ...
                <MiniPill>{metrics.linkedDebtCount} debt-linked</MiniPill>
                <MiniPill tone={nextDueTone}>
          ...
          {status ? (
            <GlassPane tone="green" size="card">
              <div style={{ fontWeight: 800, fontSize: 14, color: "#9ef0c0" }}>
                {status}
              </div>
            </GlassPane>
          ) : null}
          ...
          <section className="billMetricGrid">
            ...
            <StatCard
              icon={Link2}
              label="Linked Debt Bills"
              value={String(metrics.linkedDebtCount)}
              detail="Bills that auto-sync into a debt profile."
              tone={metrics.linkedDebtCount > 0 ? "amber" : "neutral"}
            />
            <StatCard
              icon={ArrowUpRight}
              label="Next Bill"
          ...
          <GlassPane size="card">
            <PaneHeader
              ...
              right={
                <ActionBtn variant="primary" onClick={openAddDrawer}>
                  <Plus size={14} />
                  New Bill
                </ActionBtn>
              }
          ...
          <section className="billWorkspaceGrid">
            <GlassPane size="card" style={{ height: "100%" }}>
              <div className="billRosterPane">
                <PaneHeader
                  title="Bills Roster"
                  subcopy="Click a bill to focus it. No cluttered row tools."
                ...
                {visibleBills.length ? (
                  ...
                  <div className="billEmptyState billGrowEmpty">
                    <div>
            ...
            <BillSummaryCard
              ...
              onToggle={() => selectedBill && toggleBillActive(selectedBill)}
              onDelete={deleteBill}
              onOpenHistory={() => setHistoryOpen(true)}
              onCloseHistory={() => setHistoryOpen(false)}
              historyOpen={historyOpen}
              paymentBusy={paymentDraft.saving}
            />

            <div className="billRailStack">
              <RailCard
                title="Quick Tools"
                subcopy="Keep the page short and open forms only when you need them."
              >
                <div className="billActionGrid billActionGridSingle">
                  <ActionBtn variant="primary" onClick={openAddDrawer} full>
                    <Plus size={14} />
                    New Bill
                  </ActionBtn>

                  <ActionBtn onClick={openEditDrawer} full disabled={!selectedBill}>
                    <PencilLine size={14} />
                    Edit Selected Bill
                  </ActionBtn>
                </div>
              ...
              <RailCard
                title="Selected Bill Snapshot"
                subcopy="The fast pulse of the currently focused bill."
                right={
                  selectedBill ? (
                    <MiniPill tone={billStatusMeta(selectedBill).tone}>
                      {billStatusMeta(selectedBill).label}
                    </MiniPill>
                  ) : null
                }
              >
                {selectedBill ? (
                  <div className="billInsightList">
                    <div className="billInsightItem">
                      <div className="billInsightTitle">Bill</div>
                      <div className="billInsightValue">{selectedBill.name}</div>
                      <div className="billInsightSub">
                        {money(selectedBill.amount)} •{" "}
                        {FREQUENCY_OPTIONS.find((opt) => opt.value === selectedBill.frequency)
                          ?.label || selectedBill.frequency}
                      </div>
                    </div>

                    <div className="billInsightItem">
                      <div className="billInsightTitle">Linked account</div>
                      <div className="billInsightValue">
                        {accounts.find((a) => a.id === selectedBill.accountId)?.name || "None"}
                      </div>
                      <div className="billInsightSub">
                        {selectedBill.autopay ? "Autopay on" : "Autopay off"}
                      </div>
                    </div>

                    <div className="billInsightItem">
                      <div className="billInsightTitle">Linked debt</div>
                      <div className="billInsightValue">
                        {selectedLinkedDebt?.name || "None"}
                      </div>
                      <div className="billInsightSub">
                        {selectedLinkedDebt
                          ? `${money(selectedLinkedDebt.balance)} • ${safeNum(
                              selectedLinkedDebt.aprPct,
                              0
                            )}% APR`
                          : "No debt connection"}
                      </div>
                  ...
                  <div className="billEmptyMini">No bill selected.</div>
                )}
              </RailCard>

              <RailCard
                title="Debt Snapshot"
                subcopy="If this bill pays down debt, keep the debt story visible."
              >
                {selectedLinkedDebt ? (
                  <div className="billInsightList">
                    <div className="billInsightItem">
                      <div className="billInsightTitle">Balance</div>
                      <div className="billInsightValue">
                        {money(selectedLinkedDebt.balance)}
                      </div>
                      <div className="billInsightSub">
                        Estimated interest {moneyTight(
                          estimatedMonthlyInterest(
                            selectedLinkedDebt.balance,
                            selectedLinkedDebt.aprPct
                          )
                        )}/mo
                      </div>
                    ...
                    <div className="billInsightItem">
                      <div className="billInsightTitle">Payoff</div>
                      <div className="billInsightValue">
                        {payoffLabel(
                          selectedLinkedDebt.balance,
                          selectedLinkedDebt.aprPct,
                          safeNum(selectedLinkedDebt.minPay, 0) +
                            safeNum(selectedLinkedDebt.extraPay, 0)
                        )}
                      </div>
                      <div className="billInsightSub">
                        Plan {moneyTight(
                  ...
                  <div className="billEmptyMini">
                    No debt attached to the selected bill.
                  </div>
                )}
              </RailCard>
            </div>
...
const globalStyles = `
  ...
  .billControlsGrid {
    display: grid;
    grid-template-columns: minmax(0, 1.05fr) minmax(0, 1fr) minmax(260px, 0.58fr);
    gap: 14px;
    align-items: end;
  }
  ...
  .billWorkspaceGrid {
    display: grid;
    grid-template-columns: minmax(360px, 0.9fr) minmax(640px, 1.38fr) minmax(320px, 0.78fr);
    gap: 14px;
    align-items: stretch;
  }
  ...
  @media (max-width: 1560px) {
    .billWorkspaceGrid {
      grid-template-columns: minmax(320px, 0.88fr) minmax(560px, 1.2fr) minmax(300px, 0.74fr);
    }
  }
